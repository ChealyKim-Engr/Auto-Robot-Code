...
int main (void) {
	...
	unsigned long speedAA=20000, speedBB=20000;
	signed long Ep=0, Ed=0, Ei=0, lastError=0;
	double Kp=1.4, Ki=0.4, Kd=1.4;  //1.4,0.4,1.4
  ...
  GPIO_PORTF_DATA_R = 0x04;		 // Blue led																
	...
	PWM0A_Duty(speedAA);   			 //40,000 == 100%
	PWM0B_Duty(speedBB);		       //40,000 == 100% 	
  GPIO_PORTF_DATA_R = 0x04;		 // Blue led
	...
	while ((goal == 0)||(goal == 2)) {
		...
		while (turn != 'U') {	
			while (turn == 0){ 
				...
				//***turn RF == 0x03 cyane
				if ((distance0>100)&&(distance0<400) // 120
          ...
          out = 0;	turnLeft=0; turnRight=0;				
				...
				//***turn LR == 0x06 yellow
				else if ((distance0>20)&&(distance0<90)
          ...
          out = 0; //turnLeft=0; turnRight=0;				
				...
				//STOP
				else if ((distance0>20)&&(distance0<50)
            ...
            SysTick_Wait10ms(100);     //wait
				...
				//turn RIGTH
				else if(((distance0>30)&&(distance0<70) 
								&&(distance1>150)&&(distance1<500)
								&&(distance2>20)&&(distance2<70) 
								&&(distance1>distance2))
								||(r == 1)){//RIGHT TURN																	         
					stop();	
          ...
          count = 0;									
					...
					if (goTurn==1) {
						F_R_L_Sensor();
						if ((distance0<45)||(goal==2)) { //40
							stepBack();
							SysTick_Wait10ms(5); //5
							stop();	
            }
            F_R_L_Sensor();
            if (distance0>50) {  //50
          ...
          count = 0;	out = 0; F_R_L_Sensor();				
					while((count<23)&&(out==0)){  //PID 25
						if (distance0>60) {
							moveStraight();
							SysTick_Wait10ms(1);
						}						
						if ((distance0>60)&&(distance1>100)
							  &&(distance2<100)){
							if((distance0>50)&&(distance2<50)){
            ...
            else if ((distance0>60)&&(distance1<100)
							       &&(distance2<100)) {
							GPIO_PORTF_DATA_R = 0x0E;								
							moveStraight();
            }							
            else if ((distance0>60)&&(distance2>100)
							       &&(distance1<80)) {
              if ((distance0>50)&&(distance1>50)) {
								GPIO_PORTF_DATA_R = 0x0C; //rightMove();
								distance2 = 40;
				...
				//turn LEFT
				else if (((distance0>20)&&(distance0<70)
								 &&(distance1>20)&&(distance1<70)
								 &&(distance2>150)&&(distance2<500) 
								 &&(distance2>distance1))
								 ||(l == 1)){//LEFT TURN										 
          stop();		 
          ...
          count = 0;	out = 0; F_R_L_Sensor();			
              ...
              if (distance2>50) {
					...
					while((count<23)&&(out==0)){	//PID 25
						...
						if ((distance0>60)&&(distance2>100)
							   &&(distance1<80)){
							if(distance1<50){
								GPIO_PORTF_DATA_R = 0x08;//leftMove();											 
									distance2 = 60;
								  myPIDcontrol (Kp, Ki, Kd);
							}
							else if (distance1>50){	 
...
void moveStraight(void) {
  GPIO_PORTB_DATA_R |= 0x01;//MotorA, foward	
	GPIO_PORTB_DATA_R |= 0x02;//MotorB, foward	
	PWM0A_Duty(5000);   			//40,000 == 100% , 8000
